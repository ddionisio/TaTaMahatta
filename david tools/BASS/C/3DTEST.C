/* BASS 3D Test, copyright (c) 1999-2003 Ian Luck.
==================================================
				...
				case ID_ADD:
					{
						...
						if (GetOpenFileName(&ofn)) {
							...
							if (newchan=BASS_MusicLoad(FALSE,file,0,0,BASS_MUSIC_RAMP|BASS_MUSIC_LOOP|BASS_MUSIC_3D)) {
								...
								BASS_ChannelSet3DAttributes(newchan,-1,35.0,-1.0,-1,-1,-1);
							} else
							/* Load a sample from "file" with 3D enabled, and make it loop */
							if (newchan=BASS_SampleLoad(FALSE,file,0,0,1,BASS_SAMPLE_LOOP|BASS_SAMPLE_3D)) {
								Channel *c;
								BASS_SAMPLE sam;
								chanc++;
								chans=(Channel*)realloc((void*)chans,chanc*sizeof(Channel));
		...
		case WM_HSCROLL:
			{
				...
				PostMessage((HWND)l,SBM_SETPOS,a,1);
				switch (GetDlgCtrlID((HWND)l)) {
					case ID_ROLLOFF:
						/* change the rolloff factor */
						BASS_Set3DFactors(-1.0,pow(2.0,(float)(a-10)/6.0),-1.0);
						break;
					case ID_DOPPLER:
						/* change the doppler factor */
						BASS_Set3DFactors(-1.0,-1.0,pow(2.0,(float)(a-10)/6.0));
						break;
				}
			}
			return 1;
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (!BASS_Init(device,44100,BASS_DEVICE_3D,win)) {
